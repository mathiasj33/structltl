...
class HOAWriter:
    ...
    def write_header(self):
        ...
        self.write_line(
            "AP: {} {}".format(len(props), " ".join([f'"{p}"' for p in props]))
        )
    ...
    def label_to_index_label(self, label: str | None) -> str:
        if label is None:
            return str(len(self.ldba.propositions))
        regexp = r"([a-z][a-z0-9_]*)"  # matches a single proposition
        return re.sub(
            regexp, lambda m: str(self.proposition_to_index(m.group(0))), label
        )
